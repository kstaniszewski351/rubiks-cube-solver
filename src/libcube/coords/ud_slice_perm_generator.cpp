...
{
    return lehmerEncode(cube.edgePerm.begin() + ud_slice_start, cube.edgePerm.end());
}
...
void UDSlicePermGenerator::invertCoord(int coord, CubieCube& cube) const
{
    ...
    {
        cube.edgePerm[i] = static_cast<Edge>(static_cast<int>(cube.edgePerm[i]) +  ud_slice_start);
    }
}
int UDSlicePermGenerator::getMaxCoord() const
{
    return n_coords_;
}